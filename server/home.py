...
class Home():
        ...
        try:
            ...
            if(self.Name_in_db(device_name)):
                ...
                if(device_type in ["outlet", "light"]):
                    ...
                    if(device_db[device_name]['sample_time'] - time.gmtime() <= SAMPLE_TTL):
                        curr_level = device_db[device_name]['level']
                        # return level
                        log("current level of \"" + device_name +
                            "\" is " + str(curr_level))
                        return(curr_level)
                    # if sample is expired, force a sample
                        ...
                        self._zb.remote_at(
                            dest_addr_long=bytes_mac, command='IS')
                        ...
                        while(device_db[device_name]['sample_time'] - time.gmtime() > SAMPLE_TTL):
                            # check if timed out
                            if(time.gmtime() - start_time >= SAMPLE_TIMEOUT):
                                log("could not reach device \"" + device_name +
                                    "\", timed out while waiting for sample")
                                return -1
    ...
    """
    def Set_device_stat(device_name, level):
        # get lock
        self._lock.acquire()
        try:
            ...
            if(0 <= level and level <= 100):
                ...
                if(self.Name_in_db(device_name)):
                    # get device type
                    device_type = device_db[device_name]['type']
                    ...
                    if(device_type == "outlet"):
                        ...
                        if(level == 0):
                            ...
                            if(curr_level == 100):
                                # make DIO0 high
                                xbee.remote_at(dest_addr_long=self.Mac2bytes(
                                    device_db['mac']), command='D0', parameter=b'\x05')
                                time.sleep(DIO_SIG_PERIOD)
                                # make DIO0 low
                                xbee.remote_at(dest_addr_long=self.Mac2bytes(
                                    device_db['mac']), command='D0', parameter=b'\x04')
                                # update db
                                device_db[device_name]['level'] = 0
                                device_db[device_name]['sample_time'] = time.gmtime(
                                )
                                with open(DB_FILENAME, 'w') as f:
                                    json.dump(device_db, f)
                                log("changed device \"" +
                            ...
                            if(curr_level == 0):
                                ...
                                xbee.remote_at(dest_addr_long=self.Mac2bytes(
                                    device_db['mac']), command='D0', parameter=b'\x04')
                                # update db
                                device_db[device_name]['level'] = 100
                                device_db[device_name]['sample_time'] = time.gmtime()
                                with open(DB_FILENAME, 'w') as f:
                                    json.dump(device_db, f)
                    ...
                    log("could not change level of device \"" +
                        device_name + "\", device not in db")
                    return False
    ...
    def Mac_in_db(device_mac):
        ...
        try:
            if(byte_format):
                for device in device_db.keys()):
                    if(self.Mac2bytes(device_db['mac']) == device_mac):
                        return True
    ...
    def Add_device(device_name, device_mac, device_type):
        ...
        try:
            ...
            # add to db dict
            device_db[device_name] = {'name':device_name, 'id':device_mac, 'type':device_type, 'level':-1, 'sample_time':0}
